...
/*
 * Function for getting a list of reports in a site repository
 * args input
 *      db
 *          host
 *          name
        ...
        if(!error){
            if(response.statusCode==200){
                ...
                args.site.list=response.body.rows.map(function(e){
                    var total = e.doc.statuscode.reduce(function(f,g){
                            return f+g[1];
                        },0)
                      , packet={
                            id:e.id
                          , ts_created:e.doc.ts_created
                          , pages:e.doc.hashes.length
                          , files:total-e.doc.hashes.length
                          , total:total
                        }
